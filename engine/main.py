...
#object init
cube = meshObject(0, "engine/assets/cube.obj", [10, 0, 10, 1], [0, 0, 0, 1])
axis = meshObject(1, "engine/assets/axis.obj", [-10, 0, 10, 1], [0, 0, 0, 1])
objectList = [cube, axis]
...
while running == True:
    ...
    for obj in objectList:
        ...
        matRotX = makeXRotMatrix(obj.rotation[0])
        matRotY = makeYRotMatrix(obj.rotation[1])
        matRotZ = makeZRotMatrix(obj.rotation[2])
        matTrans = makeTranslationMatrix(vecSub(obj.position, camera))
        matProj = makeProjMatrix(fov, (screen.get_height() / screen.get_width()), 0.1, 1000)
        
        #create world matrix
        matWorld = makeIdentityMatrix()
        matWorld = matrixMultMatrix(matRotX, matRotZ)
        ...
        for tri in obj.mesh:
            ...
            if vecDotProduct(normal, cameraRay) < 0:
                ...
                #loop over all triangles since some mmight have been created during clipping
